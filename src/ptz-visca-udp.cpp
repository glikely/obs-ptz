...
void ViscaUDPTransport::receive_datagram(const QNetworkDatagram &dg)
{
	...
	case 0x0111:
		if (seq != seq_state[0] && seq != seq_state[slot]) {
			blog(LOG_DEBUG, "VISCA UDP out of seq; %i != [0]%i or [%i]%i) <-- %s", seq, seq_state[0],
			     slot, seq_state[slot], qPrintable(data.toHex(':')));
...
{
	QString new_host = obs_data_get_string(config, "udp_host");
	auto port = obs_data_get_int(config, "udp_port");
	if (new_host != host) {
		ip_address.clear();
		host = new_host;
		if (!host.isEmpty()) {
...
{
	obs_data_set_string(config, "udp_host", qPrintable(host));
...
{
	obs_properties_add_text(props, "udp_host", obs_module_text("PTZ.Device.Hostname"), OBS_TEXT_DEFAULT);
