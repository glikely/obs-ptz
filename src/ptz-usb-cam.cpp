...
public:
	V4L2Control(const std::string &device)
	{
		...
		if (fd == -1) {
			blog(LOG_ERROR, "Failed to open V4L2 device: %s",
			     device_path.c_str());
			return;
		}
...
public:
	DirectShowControl(const std::string &device)
	{
		...
		if (FAILED(hr)) {
			blog(LOG_ERROR,
			     "Failed to create device enumerator: %ld", hr);
			return;
		}
		...
		if (FAILED(hr) || !enum_moniker) {
			blog(LOG_ERROR,
			     "Failed to enumerate video devices: %ld", hr);
			dev_enum->Release();
			return;
		}
		...
		while (enum_moniker->Next(1, &moniker, nullptr) == S_OK) {
			...
			if (SUCCEEDED(hr)) {
				...
				std::string device_path1(w_device_path.begin(),
							 w_device_path.end());
				if (device_path1 == decoded_std_path) {
					hr = moniker->BindToObject(
						...
						(void **)&filter_);
					if (SUCCEEDED(hr)) {
						hr = filter_->QueryInterface(
							IID_IAMCameraControl,
							(void **)&cam_control_);
						if (FAILED(hr)) {
							blog(LOG_ERROR,
							     "Failed to get IAMCameraControl: %ld",
							     hr);
							filter_->Release();
							filter_ = nullptr;
						}
	...
	bool internal_pan(long value) override
	{
		if (!cam_control_)
			return false;
		HRESULT hr = cam_control_->Set(CameraControl_Pan, value,
					       CameraControl_Flags_Manual);
		if (FAILED(hr)) {
			blog(LOG_ERROR,
			     "Failed to set Pan: %ld (mapped value: %ld)", hr,
			     value);
			return false;
		}
		return true;
	...
	bool internal_tilt(long value) override
	{
		if (!cam_control_)
			return false;
		HRESULT hr = cam_control_->Set(CameraControl_Tilt, value,
					       CameraControl_Flags_Manual);
		if (FAILED(hr)) {
			blog(LOG_ERROR,
			     "Failed to set Tilt: %ld (mapped value: %ld)", hr,
			     value);
			return false;
		}
		return true;
	...
	bool internal_zoom(long value) override
	{
		if (!cam_control_)
			return false;
		HRESULT hr = cam_control_->Set(CameraControl_Zoom, value,
					       CameraControl_Flags_Manual);
		if (FAILED(hr)) {
			blog(LOG_ERROR,
			     "Failed to set Zoom: %ld (mapped value: %ld)", hr,
			     value);
			return false;
		}
		return true;
...
void PTZUSBCam::set_config(OBSData config)
{
	...
	OBSDataArrayAutoRelease presetArray =
		obs_data_get_array(config, "presets_memory");
	size_t count = obs_data_array_count(presetArray);
	for (size_t i = 0; i < count; ++i) {
		OBSDataAutoRelease preset = obs_data_array_item(presetArray, i);
		int p_id =
			static_cast<int>(obs_data_get_int(preset, "preset_id"));
...
PTZControl *PTZUSBCam::get_ptz_control()
{
	...
	if (src) {
		...
		if (psettings) {
#ifdef _WIN32
			video_device_id = obs_data_get_string(
				psettings, "video_device_id");
#else
			video_device_id =
				obs_data_get_string(psettings, "device_id");
#endif
		}
	}
	...
	blog(LOG_INFO, "Switching PTZ USBUVC device from %s to %s",
	     ptz_control_ == nullptr ? "null"
				     : ptz_control_->getDevicePath().c_str(),
