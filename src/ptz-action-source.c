...
static void *ptz_action_source_create(obs_data_t *settings,
				      obs_source_t *source)
{
	struct ptz_action_source_data *context =
		bzalloc(sizeof(struct ptz_action_source_data));
						...
						obs_data_t *settings)
{
	...
	for (size_t i = 0;
	     i < obs_data_array_count(device_array) && !preset_array; i++) {
	...
	if (preset_array) {
		for (size_t i = 0; i < obs_data_array_count(preset_array);
		     i++) {
			obs_data_t *preset =
				obs_data_array_item(preset_array, i);
...
static void property_set_visible(obs_properties_t *props, const char *name,
				 bool visible)
{
...
static bool ptz_action_source_test_clicked_cb(obs_properties_t *props,
...
static obs_properties_t *ptz_action_source_get_properties(void *data)
{
	...
	obs_property_t *prop = obs_properties_add_list(props, "trigger",
						       "Action Trigger",
						       OBS_COMBO_TYPE_LIST,
	...
	obs_property_set_modified_callback(prop,
					   ptz_action_source_device_changed_cb);
	...
	obs_properties_add_float_slider(props, "pan_speed", "Pan Speed", -1.0,
					1.0, 0.01);
	obs_properties_add_float_slider(props, "tilt_speed", "Tilt Speed", -1.0,
					1.0, 0.01);
