...
{
	obs_source_t *cam_source = ptz_device_find_source(device_id);
	if (!cam_source)
		return false;
...
static bool ptz_action_source_action_changed_cb(obs_properties_t *props, obs_property_t *prop, obs_data_t *settings)
{
	int64_t action = obs_data_get_int(settings, "action");
	UNUSED_PARAMETER(prop);
	property_set_visible(props, "preset_id",
			     action == PTZ_ACTION_PRESET_RECALL || action == PTZ_ACTION_PRESET_SAVE);
...
static bool ptz_action_source_test_clicked_cb(obs_properties_t *props, obs_property_t *property, void *data)
{
...
static obs_properties_t *ptz_action_source_get_properties(void *data)
{
	...
	obs_property_t *prop =
		obs_properties_add_list(props, "trigger", "Action Trigger", OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	obs_property_list_add_int(prop, "Scene becomes active program", PTZ_ACTION_TRIGGER_PROGRAM_ACTIVE);
	obs_property_list_add_int(prop, "Scene becomes active preview", PTZ_ACTION_TRIGGER_PREVIEW_ACTIVE);
