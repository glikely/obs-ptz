...
void OnvifMediaProbe::onReplyFinished(QNetworkReply *reply)
{
	...
	if (!parsed) {
		m_step = Step::Idle;
		/* Write the unparseable response somewhere we can point the user
		 * at, but use QTemporaryFile in TempLocation rather than a fixed
		 * /tmp path. The fixed path was world-readable, race-prone, and
		 * symlinkable; this picks a per-user location and a unique name. */
		QString dumpPath;
		QString tempBase = QStandardPaths::writableLocation(QStandardPaths::TempLocation) +
				   "/onvif-probe-bad-response-XXXXXX.xml";
		QTemporaryFile dump(tempBase);
		dump.setAutoRemove(false);
		if (dump.open()) {
			dump.write(rawPayload);
			dumpPath = dump.fileName();
			dump.close();
		}
		QString suffix = dumpPath.isEmpty() ? QStringLiteral("(failed to write debug dump)")
						    : QString("(body saved to %1)").arg(dumpPath);
		emit error(QString("SOAP parse error at line %1 col %2: %3 %4")
				   .arg(xmlLine)
				   .arg(xmlCol)
				   .arg(xmlErr)
				   .arg(suffix));
...
void OnvifDiscoveryDialog::onRescanClicked()
{
	if (m_discovery->isRunning())
		return;
	/* Any stream probe that's still in flight is now for a camera the
	 * user no longer cares about; cancel it so its late reply doesn't
	 * populate the dialog with stale streams for a new scan's selection. */
	m_mediaProbe->cancel();
	m_cameras.clear();
...
void OnvifDiscoveryDialog::onSelectionChanged()
{
	...
	if (items.isEmpty()) {
		/* Selection went away — any in-flight probe is now for a
		 * camera the user cleared. Cancel so a late reply doesn't
		 * race in and repopulate streams/details for nothing. */
		m_mediaProbe->cancel();
		m_okButton->setEnabled(false);
...
void OnvifDiscoveryDialog::maybeProbeStreams()
{
	if (m_selected.xaddr.isEmpty())
		return;
	/* The PR description promised support for fully anonymous cameras
	 * (empty creds ⇒ skip WS-Security), but the old early-return here
	 * suppressed the probe in that case. Always probe; OnvifMediaProbe
	 * itself drops the WS-Security header when password is empty. */
	m_streamStatus = obs_module_text("PTZ.ONVIF.Discovery.FetchingStreams");
	m_streams.clear();
	rebuildDetail();
	m_mediaProbe->fetch(m_selected.host, m_selected.port, m_userEdit->text(), m_passEdit->text());
}
...
{
	auto kv = [](const char *key_id, const QString &value) {
		return QString("%1: %2").arg(obs_module_text(key_id), value);
	};
	QStringList lines;
	lines << kv("PTZ.ONVIF.Discovery.Col.Host", m_selected.host);
	lines << kv("PTZ.ONVIF.Discovery.Col.Port", QString::number(m_selected.port));
	if (!m_selected.manufacturer.isEmpty())
		lines << kv("PTZ.ONVIF.Discovery.Col.Manufacturer", m_selected.manufacturer);
	if (!m_selected.model.isEmpty())
		lines << kv("PTZ.ONVIF.Discovery.Col.Model", m_selected.model);
	if (!m_selected.location.isEmpty())
		lines << kv("PTZ.ONVIF.Discovery.Detail.Location", m_selected.location);
	if (!m_selected.uuid.isEmpty())
		lines << kv("PTZ.ONVIF.Discovery.Detail.Uuid", m_selected.uuid);
	lines << kv("PTZ.ONVIF.Discovery.Detail.DeviceService", m_selected.xaddr);
	...
	if (!m_selected.types.isEmpty()) {
		lines << "";
		lines << QString("%1:").arg(obs_module_text("PTZ.ONVIF.Discovery.Detail.Types"));
		for (const auto &t : m_selected.types)
			lines << "  " + t;
	}
	if (!m_selected.scopes.isEmpty()) {
		lines << "";
		lines << QString("%1:").arg(obs_module_text("PTZ.ONVIF.Discovery.Detail.Scopes"));
