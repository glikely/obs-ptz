...
public:
	...
	bool pan(double value)
	{
		...
		mapped_value =
			std::clamp(mapped_value, min[PTZ_PAN], max[PTZ_PAN]);
	...
	bool tilt(double value)
	{
		...
		mapped_value =
			std::clamp(mapped_value, min[PTZ_TILT], max[PTZ_TILT]);
	...
	bool zoom(double value)
	{
		...
		mapped_value =
			std::clamp(mapped_value, min[PTZ_ZOOM], max[PTZ_ZOOM]);
	...
	bool focus(double value)
	{
		...
		mapped_value = std::clamp(mapped_value, min[PTZ_FOCUS],
					  max[PTZ_FOCUS]);
