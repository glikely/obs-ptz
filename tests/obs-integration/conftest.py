...
class World:
    ...
    def device_status(self, device_id, out_file):
        """Fetches device_id's live {"connected", "pan_pos", "tilt_pos"}
        status via tests/ui-harness/device-status-test.cpp's
        "get_device_status" test -- the only way to observe
        PTZDevice::isConnected()/its cached position from outside the
        plugin, since obs-websocket has no device list or property read
        of its own. Removes any stale out_file first so a slow dispatch
        can never be mistaken for a fresh read, then waits for the
        (re)written file before parsing it."""
        if out_file.exists():
            out_file.unlink()
