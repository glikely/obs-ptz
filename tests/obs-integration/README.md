...
7. `test_device_status.py` covers camera connect/disconnect detection and
   position-update handling, via `tests/ui-harness/device-status-test.cpp`'s
   `get_device_status` test (same `-DENABLE_UI_TESTS=ON` requirement as
   above). Connect/disconnect specifically uses its own disposable
   `flaky_ptzsim` fixture -- a second, VISCA-TCP-only `ptzsim` instance a
   test can kill and restart -- rather than the session-scoped `ptzsim`
   every other test shares, since stopping *that* one would break the
   rest of the suite.
