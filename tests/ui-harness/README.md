...
Ships with four tests: `appearance_row_sizing` (see
...
its driver, `tests/obs-integration/test_device_status.py` - also a
pytest module, for the same reason), and `move_device` (see
`move-device-test.cpp` and its driver,
`tests/obs-integration/test_absolute_relative_moves.py` - likewise a
pytest module) - see "Adding a new test" below for how to add another.
