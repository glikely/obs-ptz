...
class EditableItemDialog : public QDialog {
	...
	void BrowseClicked()
	{
		...
		QString path = OpenFile(GetMainWindow(), QTStr("Browse"),
					curPath, filter);
		if (path.isEmpty())
			return;
...
void WidgetInfo::EditListAddFiles()
{
	...
	QStringList files = OpenFiles(GetMainWindow(), title,
...
void WidgetInfo::EditListAddDir()
{
	...
	QString dir = SelectDirectory(GetMainWindow(), title,
				      QT_UTF8(default_path));
...
void WidgetInfo::EditListEdit()
{
	...
	if (type == OBS_EDITABLE_LIST_TYPE_FILES) {
		...
		if (pathDir.exists())
			path = SelectDirectory(GetMainWindow(),
					       QTStr("Browse"), item->text());
		else
			path = OpenFile(GetMainWindow(), QTStr("Browse"),
